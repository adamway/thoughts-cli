...
function checkExistingSetup(config?: ThoughtsConfig | null): {
  ...
  if (!fs.lstatSync(thoughtsDir).isDirectory()) {
    return {
      exists: true,
      isValid: false,
      message: "thoughts exists but is not a directory",
    };
  }
  ...
  const hasOldLocal =
    fs.existsSync(localPath) && fs.lstatSync(localPath).isSymbolicLink();
  ...
  const hasUser =
    fs.existsSync(userPath) && fs.lstatSync(userPath).isSymbolicLink();
  const hasShared =
    fs.existsSync(sharedPath) && fs.lstatSync(sharedPath).isSymbolicLink();
  const hasGlobal =
    fs.existsSync(globalPath) && fs.lstatSync(globalPath).isSymbolicLink();
...
function generateClaudeMd(
  ...
  const reposPath = path
    .join(thoughtsRepo, reposDir, repoName)
    .replace(os.homedir(), "~");
  const globalPath = path
    .join(thoughtsRepo, "global")
    .replace(os.homedir(), "~");
  ...
  if (fs.existsSync(preCommitPath)) {
    ...
    if (
      ...
      if (
        ...
        // For outdated hooks, just remove them
...
export async function thoughtsInitCommand(options: InitOptions): Promise<void> {
  try {
    ...
    if (!config) {
      ...
      console.log(
        chalk.gray(
          "This is where all your thoughts across all projects will be stored.",
        ),
      );
      const thoughtsRepoInput = await prompt(
        `Thoughts repository location [${defaultRepo}]: `,
      );
      ...
      console.log(
        chalk.gray(
          "Your thoughts will be organized into two main directories:",
        ),
      );
      console.log(
        chalk.gray(
          "- Repository-specific thoughts (one subdirectory per project)",
        ),
      );
      ...
      const globalDirInput = await prompt(
        `Directory name for global thoughts [global]: `,
      );
      ...
      while (!user || user.toLowerCase() === "global") {
        ...
        if (user.toLowerCase() === "global") {
          console.log(
            chalk.red(
              'Username cannot be "global" as it\'s reserved for cross-project thoughts.',
            ),
          );
          user = "";
        }
    ...
    if (options.profile) {
      if (!validateProfile(config, options.profile)) {
        console.error(
          chalk.red(`Error: Profile "${options.profile}" does not exist.`),
        );
        console.error("");
    ...
    if (setupStatus.exists && !options.force) {
      if (setupStatus.isValid) {
        console.log(
          chalk.yellow(
            "Thoughts directory already configured for this repository.",
          ),
        );
        ...
        console.log(
          chalk.yellow(
            `Warning: ${setupStatus.message || "Thoughts setup is incomplete"}`,
          ),
        );
        ...
        if (setupStatus.isOldStructure) {
          ...
          console.log(
            chalk.blue(
              "The thoughts system has been upgraded to use a simpler structure:",
            ),
          );
    ...
    if (!fs.existsSync(expandedRepo)) {
      console.log(
        chalk.red(
          `Error: Thoughts repository not found at ${tempProfileConfig.thoughtsRepo}`,
        ),
      );
      console.log(
        chalk.yellow("The thoughts repository may have been moved or deleted."),
      );
      const recreate = await prompt("Do you want to recreate it? (Y/n): ");
      if (recreate.toLowerCase() === "n") {
        console.log(
          "Please update your configuration or restore the thoughts repository.",
        );
        process.exit(1);
      }
      ensureThoughtsRepoExists(
    ...
    if (!mappedName) {
      if (options.directory) {
        ...
        if (!existingRepos.includes(sanitizedDir)) {
          console.error(
            chalk.red(
              `Error: Directory "${sanitizedDir}" not found in thoughts repository.`,
            ),
          );
          console.error(
            chalk.red(
              "In non-interactive mode (--directory), you must specify a directory",
            ),
          );
          console.error(
            chalk.red("name that already exists in the thoughts repository."),
          );
          console.error("");
        ...
        console.log(
          chalk.gray("to store thoughts specific to this repository."),
        );
        console.log("");

        if (existingRepos.length > 0) {
          console.log(
            "Select or create a thoughts directory for this repository:",
          );
          const options = [
    ...
    if (otherUsers.length > 0) {
      console.log(
        chalk.green(`Added symlinks for other users: ${otherUsers.join(", ")}`),
      );
    }
    ...
    if (hookResult.updated.length > 0) {
      console.log(
        chalk.yellow(`Updated git hooks: ${hookResult.updated.join(", ")}`),
      );
    }
