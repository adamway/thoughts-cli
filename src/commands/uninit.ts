...
export async function thoughtsUninitCommand(
  ...
  try {
    ...
    if (!mappedName && !options.force) {
      console.error(
        chalk.red(
          "Error: This repository is not in the thoughts configuration.",
        ),
      );
      console.error(
        chalk.yellow("Use --force to remove the thoughts directory anyway."),
      );
      process.exit(1);
    }

    console.log(
      chalk.blue("Removing thoughts setup from current repository..."),
    );
    ...
    try {
      ...
      console.error(
        chalk.yellow("You may need to manually remove: " + thoughtsDir),
      );
      process.exit(1);
    }
    ...
    if (mappedName) {
      console.log(
        chalk.gray("Removing repository from thoughts configuration..."),
      );
    ...
    if (mappedName) {
      ...
      if (profileName && config.profiles && config.profiles[profileName]) {
        ...
        console.log(
          chalk.gray(
            `  ${profile.thoughtsRepo}/${profile.reposDir}/${mappedName}`,
          ),
        );
        ...
        console.log(
          chalk.gray(
            `  ${config.thoughtsRepo}/${config.reposDir}/${mappedName}`,
          ),
        );
      }

      console.log(
        chalk.gray("Only the local symlinks and configuration were removed."),
      );
    }
  } catch (error) {
