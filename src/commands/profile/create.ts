...
export async function profileCreateCommand(
  ...
  try {
    ...
    if (sanitizedName !== profileName) {
      console.log(
        chalk.yellow(
          `Profile name sanitized: "${profileName}" → "${sanitizedName}"`,
        ),
      );
    }
    ...
    if (validateProfile(config, sanitizedName)) {
      console.error(
        chalk.red(`Error: Profile "${sanitizedName}" already exists.`),
      );
      console.error(
    ...
    if (options.repo && options.reposDir && options.globalDir) {
      ...
      console.log(
        chalk.gray(
          "Specify the thoughts repository location for this profile.",
        ),
      );
      ...
      const reposDirInput = await prompt(
        `Repository-specific thoughts directory [repos]: `,
      );
      reposDir = reposDirInput || "repos";

      const globalDirInput = await prompt(
        `Global thoughts directory [global]: `,
      );
