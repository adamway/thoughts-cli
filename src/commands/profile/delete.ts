...
export async function profileDeleteCommand(
  ...
  try {
    ...
    if (usingRepos.length > 0 && !options.force) {
      ...
      console.error(
        chalk.gray(
          "  2. Use --force to delete anyway (repos will fall back to default config)",
        ),
      );
      process.exit(1);
    }
    ...
    if (!options.force) {
      ...
      console.log(
        chalk.gray("The thoughts repository files will NOT be deleted."),
      );
      console.log("");
    ...
    if (usingRepos.length > 0) {
      ...
      console.log(
        chalk.yellow(
          "Warning: Repositories using this profile will fall back to default config",
        ),
      );
    }
  } catch (error) {
