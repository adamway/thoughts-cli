...
export function loadThoughtsConfig(
  options: Record<string, unknown> = {},
): ThoughtsConfig | null {
  const configFile = loadConfigFile(options.configFile as string | undefined);
  return configFile.thoughts || null;
}
...
export function saveThoughtsConfig(
  ...
  const configFile = loadConfigFile(options.configFile as string | undefined);
  configFile.thoughts = thoughtsConfig;
  saveConfigFile(configFile, options.configFile as string | undefined);
}
...
`;
    ...
    execSync('git commit -m "Initial thoughts repository setup"', {
      cwd: expandedRepo,
    });
  }
}
...
export function getRepoThoughtsPath(
  ...
  if (typeof thoughtsRepoOrConfig === "string") {
    ...
    return path.join(
      expandPath(thoughtsRepoOrConfig),
      reposDirOrRepoName,
      repoName!,
    );
  }
  ...
  return path.join(
    expandPath(config.thoughtsRepo),
    config.reposDir,
    reposDirOrRepoName,
  );
}
...
export function createThoughtsDirectoryStructure(
  ...
  const globalPath = getGlobalThoughtsPath(
    resolvedConfig.thoughtsRepo,
    resolvedConfig.globalDir,
  );
